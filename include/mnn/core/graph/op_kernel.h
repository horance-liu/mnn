...
class OpKernelContext {
 public:
  ...
  void set_in_out(const std::vector<Matrix *> &in_data,
  ...
  Matrix &input(const int idx) { return *(*in_data_)[idx]; }
  const Matrix &input(const int idx) const { return *(*in_data_)[idx]; }

  Matrix &output(const int idx) { return *(*out_data_)[idx]; }
  const Matrix &output(const int idx) const { return *(*out_data_)[idx]; }

  Matrix &input_grad(const int idx) { return *(*in_grad_)[idx]; }
  const Matrix &input_grad(const int idx) const { return *(*in_grad_)[idx]; }

  Matrix &output_grad(const int idx) { return *(*out_grad_)[idx]; }
  ...
  BackendType engine() const { return op_params_->engine; }
  void setEngine(const BackendType engine) { op_params_->engine = engine; }
